...
- FPS 绿色越高越好，表示FPS越高；上方的红色表示影响用户体验。
- CPU 当CPU充满颜色时，说明此时CPU满负荷；如果长时间处于满负荷，可能时需要优化了。
- Frames 鼠标划过，显示每一帧的时间和FPS的值
